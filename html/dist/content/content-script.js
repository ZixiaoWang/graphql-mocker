function GraphqlMockerInitializer() {
  !(function (t, e) {
  ...
  ((window) => {
    ...
    window.ah.fetch = fetch;
    
    const fetchHandler = {
      next: (config) => window.ah.fetch(config.url, config),
    ...
    window.fetch = (url, config) => {
      ...
      if (!globalFetchProxy.onRequest || typeof globalFetchProxy.onRequest !== "function") {
        return window.ah.fetch(url, config)
      } else {
        const globalConfig = {
          url,
          ...config
        };
        return globalFetchProxy.onRequest(globalConfig,fetchHandler);
      }
    }
    ...
    if (window.ah && window.ah.proxy && typeof window.ah.proxy === "function") {
      ...
      console.log("XMLHttpRequest failed to be proxied");
    }
  })(window);  
}

(() => {
  const script = docuent.createElement("script");
  script.textContent = GraphqlMockerInitializer.toString() + "\nGraphqlMockerInitializer()";
  script.onload = () => {
